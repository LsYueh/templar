...
test('COBOL-PIC : 9', { timeout }, async ({ match }) => {
    const tokens1 = CB.parsePic('9');

    match(tokens1, [
        { char: '9', count: 1 }
    ]);

    const tokens2 = CB.parsePic('9(1)');

    match(tokens2, [
        { char: '9', count: 1 }
    ]);

    const tokens3 = CB.parsePic('9(02)');
...
test('COBOL-PIC : X', { timeout }, async ({ match }) => {
    const tokens1 = CB.parsePic('X');

    match(tokens1, [
        { char: 'X', count: 1 }
    ]);

    const tokens2 = CB.parsePic('X(1)');

    match(tokens2, [
        { char: 'X', count: 1 }
    ]);

    const tokens3 = CB.parsePic('X(02)');
