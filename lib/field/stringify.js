 ...
 * @param {Buffer} buf 
 * @param {PicType_t} picType 
 * @param {boolean} [isVariableLength] 變動長度，只限制最大長度
 * @returns {string} 
 */
function toX(buf, picType, isVariableLength = false) {
    const buffer = isVariableLength ? buf.subarray(0, picType.length) : bufferPadEnd(buf, picType.length).subarray(0, picType.length);
 ...
 * @param {boolean} [options.isVariableLength] 變動長度，只限制最大長度
 * @returns 
 */
function stringify(input, options = {}) {
    const picType = options.picType;
    const overpunchOption = options.overpunchOption ?? new OverpunchOpt();
    const isVariableLength = options.isVariableLength;

    const ascii = toASCII(input, picType);

    switch (picType.type) {
        case 'X': { return toX(ascii, picType, isVariableLength); }
