...
const { PicType } = require('./cobol/cobol-pic.js');
const { SignIs, OverpunchOpt } = require('./cobol/overpunch.js');

const { DataType } = require('../spec/field.js');
...
test('Parse S9(3)', { timeout }, async ({ equal }) => {
    equal(parse({ picStr: 'S9(3)', type: DataType.S9, text: '12C' }), 123);
    equal(parse({ picStr: 'S9(3)', type: DataType.S9, text: '12L' }), -123);

    equal(parse({ picStr: 'S9(3)', type: DataType.S9, text: 'J23', overpunchOption: new OverpunchOpt(undefined, SignIs.Leading) }), -123);
});

test('Parse S9(3)V9', { timeout }, async ({ equal }) => {
    equal(parse({ picStr: 'S9(3)V9', type: DataType.S9, text: '12C' }), 12.3);
    equal(parse({ picStr: 'S9(3)V9', type: DataType.S9, text: '12L' }), -12.3);

    equal(parse({ picStr: 'S9(3)V9', type: DataType.S9, text: 'J23', overpunchOption: new OverpunchOpt(undefined, SignIs.Leading) }), -12.3);
});
