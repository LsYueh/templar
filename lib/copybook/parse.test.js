'use strict'

const iconv = require("iconv-lite");
const { test } = require('tap');

const { parse } = require('./parse.js');

const timeout = 60000;


/**------+---------+---------+---------+---------+---------+---------+----------
 * Test - Copybook
---------+---------+---------+---------+---------+---------+---------+--------*/

test('Copybook: T30', { timeout }, async (t) => {
    const text = '2330  01155000001140000001140000020250821000024BS1台積電          00000000000000000010Y0            ';

    t.throws(() => {
        parse(text, { fileCode: 'T30' });
    }, new Error(`FILE-CODE: 'T30' Size mismatch. Expected: 100, Actual: 97`));

    const T30 = parse(iconv.encode(text, 'cp950'), { fileCode: 'T30' });

    t.notMatch(T30, null);
    t.equal(T30.StockNo, '2330');
    t.equal(T30.StockName, '台積電');
    t.notHas(T30, 'IndSubCode', 'OTC only');
    t.notHas(T30, 'StkMark', 'OTC only');
    t.equal(T30.MarkDayTrade, 'Y');
    t.equal(T30.StkCTGCD, '0');
});

test('Copybook: T30 (OTC)', { timeout }, async (t) => {
    const text = '00620100023950000021780000019610020250822000000  1元大富櫃50      000000000000000000100X0           ';

    const T30 = parse(iconv.encode(text, 'cp950'), { fileCode: 'T30', market: 'OTC' });

    t.notMatch(T30, null);
    t.equal(T30.StockNo, '006201');
    t.equal(T30.StockName, '元大富櫃50');
    t.hasProp(T30, 'IndSubCode', 'OTC only');
    t.hasProp(T30, 'StkMark', 'OTC only');
    t.equal(T30.MarkDayTrade, 'X');
    t.equal(T30.StkCTGCD, '0');
});

test('Copybook: O40', { timeout }, async (t) => {
    const text = '2330  台積電          01155000001140000001140000020250821   ';

    const O40 = parse(iconv.encode(text, 'cp950'), { fileCode: 'O40' });

    t.notMatch(O40, null);
    t.equal(O40.StockNo, '2330');
    t.equal(O40.StockName, '台積電');
    t.equal(O40.RslPr, 1155.0);
    t.equal(O40.FllPr, 1140.0);
    t.equal(O40.RefPr, 1140.0);
    t.hasProp(O40, 'LMthDat');
});

test('Copybook: O40 (OTC with market override)', { timeout }, async (t) => {
    const text = '2330  台積電          01155000001140000001140000020250821   ';
    
    const data = iconv.encode(text, 'cp950');
    const options = { fileCode: 'O40', market: 'OTC' };

    t.throws(() => {
        parse(data, options);
    }, new Error(`Unknown OTC FILE-CODE: 'O40'`));

    options.marketOverride = true
    const O40 = parse(data, options);

    t.notMatch(O40, null);
    t.equal(O40.StockNo, '2330');
    t.equal(O40.StockName, '台積電');
    t.equal(O40.RslPr, 1155.0);
    t.equal(O40.FllPr, 1140.0);
    t.equal(O40.RefPr, 1140.0);
    t.hasProp(O40, 'LMthDat');
});

test('Copybook: R3', { timeout }, async (t) => {
    const text = '2330  000000011234567890059599990BA000198121370000001983N00000001 ';

    const R3 = parse(iconv.encode(text, 'cp950'), { fileCode: 'R3' });

    t.notMatch(R3, null);
    t.equal(R3.StkNo, '2330');
    t.equal(R3.MthQty, 1);
    t.equal(R3.MthPr, 12345.6789);
    t.hasProp(R3, 'MthTime');
    t.equal(R3.ExCd, '0');
    t.equal(R3.BuySell, 'B');
    t.equal(R3.OrderNo, 'A0001');
    t.equal(R3.IVAcNo, '9812137');
    t.equal(R3.OdrTpe, '0');
    t.equal(R3.SeqNo, '000001');
    t.equal(R3.BrokerId, '983N');
    t.equal(R3.RecNo, '00000001');
    t.equal(R3.MarkS, ' ');
});
